...
static const YYCODETYPE yyRuleInfoLhs[] = {
   ...
   160,  /* (229) cmd ::= VACUUM */
   160,  /* (230) cmd ::= VACUUM nm */
   160,  /* (231) cmd ::= PRAGMA nm dbnm */
   160,  /* (232) cmd ::= PRAGMA nm dbnm EQ nmnum */
   160,  /* (233) cmd ::= PRAGMA nm dbnm LP nmnum RP */
   160,  /* (234) cmd ::= PRAGMA nm dbnm EQ minus_num */
   160,  /* (235) cmd ::= PRAGMA nm dbnm LP minus_num RP */
   180,  /* (236) plus_num ::= PLUS INTEGER|FLOAT */
   181,  /* (237) minus_num ::= MINUS INTEGER|FLOAT */
   160,  /* (238) cmd ::= createkw trigger_decl BEGIN trigger_cmd_list END */
   248,  /* (239) trigger_decl ::= temp TRIGGER ifnotexists nm dbnm trigger_time trigger_event ON fullname foreach_clause when_clause */
   250,  /* (240) trigger_time ::= BEFORE|AFTER */
   250,  /* (241) trigger_time ::= INSTEAD OF */
   250,  /* (242) trigger_time ::= */
   251,  /* (243) trigger_event ::= DELETE|INSERT */
   251,  /* (244) trigger_event ::= UPDATE */
   251,  /* (245) trigger_event ::= UPDATE OF idlist */
   253,  /* (246) when_clause ::= */
   253,  /* (247) when_clause ::= WHEN expr */
   249,  /* (248) trigger_cmd_list ::= trigger_cmd_list trigger_cmd SEMI */
   249,  /* (249) trigger_cmd_list ::= trigger_cmd SEMI */
   255,  /* (250) trnm ::= nm DOT nm */
   256,  /* (251) tridxby ::= INDEXED BY nm */
   256,  /* (252) tridxby ::= NOT INDEXED */
   254,  /* (253) trigger_cmd ::= UPDATE orconf trnm tridxby SET setlist where_opt scanpt */
   254,  /* (254) trigger_cmd ::= scanpt insert_cmd INTO trnm idlist_opt select upsert scanpt */
   254,  /* (255) trigger_cmd ::= DELETE FROM trnm tridxby where_opt scanpt */
   254,  /* (256) trigger_cmd ::= scanpt select scanpt */
   185,  /* (257) expr ::= RAISE LP IGNORE RP */
   185,  /* (258) expr ::= RAISE LP raisetype COMMA nm RP */
   203,  /* (259) raisetype ::= ROLLBACK */
   203,  /* (260) raisetype ::= ABORT */
   203,  /* (261) raisetype ::= FAIL */
   160,  /* (262) cmd ::= DROP TRIGGER ifexists fullname */
   160,  /* (263) cmd ::= ATTACH database_kw_opt expr AS expr key_opt */
   160,  /* (264) cmd ::= DETACH database_kw_opt expr */
   258,  /* (265) key_opt ::= */
   258,  /* (266) key_opt ::= KEY expr */
   160,  /* (267) cmd ::= REINDEX */
   160,  /* (268) cmd ::= REINDEX nm dbnm */
   160,  /* (269) cmd ::= ANALYZE */
   160,  /* (270) cmd ::= ANALYZE nm dbnm */
   160,  /* (271) cmd ::= ALTER TABLE fullname RENAME TO nm */
   160,  /* (272) cmd ::= ALTER TABLE add_column_fullname ADD kwcolumn_opt columnname carglist */
   259,  /* (273) add_column_fullname ::= fullname */
   160,  /* (274) cmd ::= ALTER TABLE fullname RENAME kwcolumn_opt nm TO nm */
   160,  /* (275) cmd ::= create_vtab */
   160,  /* (276) cmd ::= create_vtab LP vtabarglist RP */
   261,  /* (277) create_vtab ::= createkw VIRTUAL TABLE ifnotexists nm dbnm USING nm */
   263,  /* (278) vtabarg ::= */
   264,  /* (279) vtabargtoken ::= ANY */
   264,  /* (280) vtabargtoken ::= lp anylist RP */
   265,  /* (281) lp ::= LP */
   232,  /* (282) with ::= WITH wqlist */
   232,  /* (283) with ::= WITH RECURSIVE wqlist */
   208,  /* (284) wqlist ::= nm eidlist_opt AS LP select RP */
   208,  /* (285) wqlist ::= wqlist COMMA nm eidlist_opt AS LP select RP */
   267,  /* (286) windowdefn_list ::= windowdefn */
   267,  /* (287) windowdefn_list ::= windowdefn_list COMMA windowdefn */
   268,  /* (288) windowdefn ::= nm AS window */
   269,  /* (289) window ::= LP part_opt orderby_opt frame_opt RP */
   271,  /* (290) part_opt ::= PARTITION BY nexprlist */
   271,  /* (291) part_opt ::= */
   270,  /* (292) frame_opt ::= */
   270,  /* (293) frame_opt ::= range_or_rows frame_bound_s */
   270,  /* (294) frame_opt ::= range_or_rows BETWEEN frame_bound_s AND frame_bound_e */
   273,  /* (295) range_or_rows ::= RANGE */
   273,  /* (296) range_or_rows ::= ROWS */
   275,  /* (297) frame_bound_s ::= frame_bound */
   275,  /* (298) frame_bound_s ::= UNBOUNDED PRECEDING */
   276,  /* (299) frame_bound_e ::= frame_bound */
   276,  /* (300) frame_bound_e ::= UNBOUNDED FOLLOWING */
   274,  /* (301) frame_bound ::= expr PRECEDING */
   274,  /* (302) frame_bound ::= CURRENT ROW */
   274,  /* (303) frame_bound ::= expr FOLLOWING */
   218,  /* (304) window_clause ::= WINDOW windowdefn_list */
   237,  /* (305) over_clause ::= filter_opt OVER window */
   237,  /* (306) over_clause ::= filter_opt OVER nm */
   272,  /* (307) filter_opt ::= */
   272,  /* (308) filter_opt ::= FILTER LP WHERE expr RP */
   155,  /* (309) input ::= cmdlist */
   156,  /* (310) cmdlist ::= cmdlist ecmd */
   156,  /* (311) cmdlist ::= ecmd */
   157,  /* (312) ecmd ::= SEMI */
   157,  /* (313) ecmd ::= cmdx SEMI */
   157,  /* (314) ecmd ::= explain cmdx */
   162,  /* (315) trans_opt ::= */
   162,  /* (316) trans_opt ::= TRANSACTION */
   162,  /* (317) trans_opt ::= TRANSACTION nm */
   164,  /* (318) savepoint_opt ::= SAVEPOINT */
   164,  /* (319) savepoint_opt ::= */
   160,  /* (320) cmd ::= create_table create_table_args */
   171,  /* (321) columnlist ::= columnlist COMMA columnname carglist */
   171,  /* (322) columnlist ::= columnname carglist */
   163,  /* (323) nm ::= ID|INDEXED */
   163,  /* (324) nm ::= STRING */
   163,  /* (325) nm ::= JOIN_KW */
   177,  /* (326) typetoken ::= typename */
   178,  /* (327) typename ::= ID|STRING */
   179,  /* (328) signed ::= plus_num */
   179,  /* (329) signed ::= minus_num */
   176,  /* (330) carglist ::= carglist ccons */
   176,  /* (331) carglist ::= */
   183,  /* (332) ccons ::= NULL onconf */
   172,  /* (333) conslist_opt ::= COMMA conslist */
   195,  /* (334) conslist ::= conslist tconscomma tcons */
   195,  /* (335) conslist ::= tcons */
   196,  /* (336) tconscomma ::= */
   200,  /* (337) defer_subclause_opt ::= defer_subclause */
   202,  /* (338) resolvetype ::= raisetype */
   206,  /* (339) selectnowith ::= oneselect */
   207,  /* (340) oneselect ::= values */
   221,  /* (341) sclp ::= selcollist COMMA */
   222,  /* (342) as ::= ID|STRING */
   185,  /* (343) expr ::= term */
   238,  /* (344) likeop ::= LIKE_KW|MATCH */
   229,  /* (345) exprlist ::= nexprlist */
   247,  /* (346) nmnum ::= plus_num */
   247,  /* (347) nmnum ::= nm */
   247,  /* (348) nmnum ::= ON */
   247,  /* (349) nmnum ::= DELETE */
   247,  /* (350) nmnum ::= DEFAULT */
   180,  /* (351) plus_num ::= INTEGER|FLOAT */
   252,  /* (352) foreach_clause ::= */
   252,  /* (353) foreach_clause ::= FOR EACH ROW */
   255,  /* (354) trnm ::= nm */
   256,  /* (355) tridxby ::= */
   257,  /* (356) database_kw_opt ::= DATABASE */
   257,  /* (357) database_kw_opt ::= */
   260,  /* (358) kwcolumn_opt ::= */
   260,  /* (359) kwcolumn_opt ::= COLUMNKW */
   262,  /* (360) vtabarglist ::= vtabarg */
   262,  /* (361) vtabarglist ::= vtabarglist COMMA vtabarg */
   263,  /* (362) vtabarg ::= vtabarg vtabargtoken */
   266,  /* (363) anylist ::= */
   266,  /* (364) anylist ::= anylist LP anylist RP */
   266,  /* (365) anylist ::= anylist ANY */
   232,  /* (366) with ::= */
};
...
static const signed char yyRuleInfoNRhs[] = {
  ...
  -12,  /* (219) cmd ::= createkw uniqueflag INDEX ifnotexists nm dbnm ON nm LP sortlist RP where_opt */
   ...
   -1,  /* (229) cmd ::= VACUUM */
   -2,  /* (230) cmd ::= VACUUM nm */
   -3,  /* (231) cmd ::= PRAGMA nm dbnm */
   -5,  /* (232) cmd ::= PRAGMA nm dbnm EQ nmnum */
   -6,  /* (233) cmd ::= PRAGMA nm dbnm LP nmnum RP */
   -5,  /* (234) cmd ::= PRAGMA nm dbnm EQ minus_num */
   -6,  /* (235) cmd ::= PRAGMA nm dbnm LP minus_num RP */
   -2,  /* (236) plus_num ::= PLUS INTEGER|FLOAT */
   -2,  /* (237) minus_num ::= MINUS INTEGER|FLOAT */
   -5,  /* (238) cmd ::= createkw trigger_decl BEGIN trigger_cmd_list END */
  -11,  /* (239) trigger_decl ::= temp TRIGGER ifnotexists nm dbnm trigger_time trigger_event ON fullname foreach_clause when_clause */
   -1,  /* (240) trigger_time ::= BEFORE|AFTER */
   -2,  /* (241) trigger_time ::= INSTEAD OF */
    0,  /* (242) trigger_time ::= */
   -1,  /* (243) trigger_event ::= DELETE|INSERT */
   -1,  /* (244) trigger_event ::= UPDATE */
   -3,  /* (245) trigger_event ::= UPDATE OF idlist */
    0,  /* (246) when_clause ::= */
   -2,  /* (247) when_clause ::= WHEN expr */
   -3,  /* (248) trigger_cmd_list ::= trigger_cmd_list trigger_cmd SEMI */
   -2,  /* (249) trigger_cmd_list ::= trigger_cmd SEMI */
   -3,  /* (250) trnm ::= nm DOT nm */
   -3,  /* (251) tridxby ::= INDEXED BY nm */
   -2,  /* (252) tridxby ::= NOT INDEXED */
   -8,  /* (253) trigger_cmd ::= UPDATE orconf trnm tridxby SET setlist where_opt scanpt */
   -8,  /* (254) trigger_cmd ::= scanpt insert_cmd INTO trnm idlist_opt select upsert scanpt */
   -6,  /* (255) trigger_cmd ::= DELETE FROM trnm tridxby where_opt scanpt */
   -3,  /* (256) trigger_cmd ::= scanpt select scanpt */
   -4,  /* (257) expr ::= RAISE LP IGNORE RP */
   -6,  /* (258) expr ::= RAISE LP raisetype COMMA nm RP */
   -1,  /* (259) raisetype ::= ROLLBACK */
   -1,  /* (260) raisetype ::= ABORT */
   -1,  /* (261) raisetype ::= FAIL */
   -4,  /* (262) cmd ::= DROP TRIGGER ifexists fullname */
   -6,  /* (263) cmd ::= ATTACH database_kw_opt expr AS expr key_opt */
   -3,  /* (264) cmd ::= DETACH database_kw_opt expr */
    0,  /* (265) key_opt ::= */
   -2,  /* (266) key_opt ::= KEY expr */
   -1,  /* (267) cmd ::= REINDEX */
   ...
   -3,  /* (288) windowdefn ::= nm AS window */
   -5,  /* (289) window ::= LP part_opt orderby_opt frame_opt RP */
   -3,  /* (290) part_opt ::= PARTITION BY nexprlist */
    0,  /* (291) part_opt ::= */
    0,  /* (292) frame_opt ::= */
   -2,  /* (293) frame_opt ::= range_or_rows frame_bound_s */
   -5,  /* (294) frame_opt ::= range_or_rows BETWEEN frame_bound_s AND frame_bound_e */
   -1,  /* (295) range_or_rows ::= RANGE */
   -1,  /* (296) range_or_rows ::= ROWS */
   -1,  /* (297) frame_bound_s ::= frame_bound */
   -2,  /* (298) frame_bound_s ::= UNBOUNDED PRECEDING */
   -1,  /* (299) frame_bound_e ::= frame_bound */
   -2,  /* (300) frame_bound_e ::= UNBOUNDED FOLLOWING */
   -2,  /* (301) frame_bound ::= expr PRECEDING */
   -2,  /* (302) frame_bound ::= CURRENT ROW */
   -2,  /* (303) frame_bound ::= expr FOLLOWING */
   -2,  /* (304) window_clause ::= WINDOW windowdefn_list */
   -3,  /* (305) over_clause ::= filter_opt OVER window */
   -3,  /* (306) over_clause ::= filter_opt OVER nm */
    0,  /* (307) filter_opt ::= */
   -5,  /* (308) filter_opt ::= FILTER LP WHERE expr RP */
   -1,  /* (309) input ::= cmdlist */
   -2,  /* (310) cmdlist ::= cmdlist ecmd */
   -1,  /* (311) cmdlist ::= ecmd */
   -1,  /* (312) ecmd ::= SEMI */
   -2,  /* (313) ecmd ::= cmdx SEMI */
   -2,  /* (314) ecmd ::= explain cmdx */
    0,  /* (315) trans_opt ::= */
   -1,  /* (316) trans_opt ::= TRANSACTION */
   -2,  /* (317) trans_opt ::= TRANSACTION nm */
   -1,  /* (318) savepoint_opt ::= SAVEPOINT */
    0,  /* (319) savepoint_opt ::= */
   -2,  /* (320) cmd ::= create_table create_table_args */
   -4,  /* (321) columnlist ::= columnlist COMMA columnname carglist */
   -2,  /* (322) columnlist ::= columnname carglist */
   -1,  /* (323) nm ::= ID|INDEXED */
   -1,  /* (324) nm ::= STRING */
   -1,  /* (325) nm ::= JOIN_KW */
   -1,  /* (326) typetoken ::= typename */
   -1,  /* (327) typename ::= ID|STRING */
   -1,  /* (328) signed ::= plus_num */
   -1,  /* (329) signed ::= minus_num */
   -2,  /* (330) carglist ::= carglist ccons */
    0,  /* (331) carglist ::= */
   -2,  /* (332) ccons ::= NULL onconf */
   -2,  /* (333) conslist_opt ::= COMMA conslist */
   -3,  /* (334) conslist ::= conslist tconscomma tcons */
   -1,  /* (335) conslist ::= tcons */
    0,  /* (336) tconscomma ::= */
   -1,  /* (337) defer_subclause_opt ::= defer_subclause */
   -1,  /* (338) resolvetype ::= raisetype */
   -1,  /* (339) selectnowith ::= oneselect */
   -1,  /* (340) oneselect ::= values */
   -2,  /* (341) sclp ::= selcollist COMMA */
   -1,  /* (342) as ::= ID|STRING */
   -1,  /* (343) expr ::= term */
   -1,  /* (344) likeop ::= LIKE_KW|MATCH */
   -1,  /* (345) exprlist ::= nexprlist */
   -1,  /* (346) nmnum ::= plus_num */
   -1,  /* (347) nmnum ::= nm */
   -1,  /* (348) nmnum ::= ON */
   -1,  /* (349) nmnum ::= DELETE */
   -1,  /* (350) nmnum ::= DEFAULT */
   -1,  /* (351) plus_num ::= INTEGER|FLOAT */
    0,  /* (352) foreach_clause ::= */
   -3,  /* (353) foreach_clause ::= FOR EACH ROW */
   -1,  /* (354) trnm ::= nm */
    0,  /* (355) tridxby ::= */
   -1,  /* (356) database_kw_opt ::= DATABASE */
    0,  /* (357) database_kw_opt ::= */
    0,  /* (358) kwcolumn_opt ::= */
   -1,  /* (359) kwcolumn_opt ::= COLUMNKW */
   -1,  /* (360) vtabarglist ::= vtabarg */
   -3,  /* (361) vtabarglist ::= vtabarglist COMMA vtabarg */
   -2,  /* (362) vtabarg ::= vtabarg vtabargtoken */
    0,  /* (363) anylist ::= */
   -4,  /* (364) anylist ::= anylist LP anylist RP */
   -2,  /* (365) anylist ::= anylist ANY */
    0,  /* (366) with ::= */
};

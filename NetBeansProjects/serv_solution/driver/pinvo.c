...
static char *split_command(char *obje, char *metho, char *line_stri) {
    char *parse_tok = kill_blank(line_stri);
    //  0x00:only method / command 0x01:object method
    switch (comd_meth_type(parse_tok)) {
        case 0x00:
            strcpy(obje, OSDB_DEFAULT_VALUE);
...
static int bulid_invok_paras(osv_pstyl_t *pscon, char *para_str) {
    ...
    //
    unsigned short colen = encode_dist(pscon->pstyl_str + sizeof (obje_entry), invo_paras);
...
int parse_invok_metho(osv_pstyl_t *pscon, char *para_str) {
    ...
    char *parse_tok = split_command(obje_iden, comd_meth, para_str);
    if (!parse_tok) return -1;
    //
    unsigned short enlen = 0x00;
    unsigned short colen = strlen(obje_iden) + 0x01;
    char *last_str = stri_pstyl_ex(pscon->pstyl_str + sizeof (obje_entry), obje_iden, colen);
    enlen += sizeof (stri_entry) + colen;
    colen = strlen(comd_meth) + 0x01;
    last_str = stri_pstyl_ex(last_str, comd_meth, colen);
    enlen += sizeof (stri_entry) + colen;
    //
    osv_pstyl_t ivkpa;

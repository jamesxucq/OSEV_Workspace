...
int system_config_parser(struct serv_config *config, xmlDocPtr docXml, xmlNodePtr curNode) {
    xmlChar *sValue = NULL;
    ...
    while (NULL != curNode) {
        if (!xmlStrcmp(curNode->name, (const xmlChar *) "User")) {
            ...
            xmlFree(sValue);
        } else if (!xmlStrcmp(curNode->name, (const xmlChar *) "WorkerProcesses")) {
            sValue = xmlNodeGetContent(curNode);
            config->worker_processes = atoi((char *) sValue);
            xmlFree(sValue);
...
int server_config_parser(struct serv_config *config, xmlDocPtr docXml, xmlNodePtr curNode) {
    xmlChar *sValue = NULL;
    /* We don't care what the top level element name is */
    curNode = curNode->xmlChildrenNode;
    while (NULL != curNode) {
...
int network_config_parser(struct serv_config *config, xmlDocPtr docXml, xmlNodePtr curNode) {
    xmlChar *sValue = NULL;
    /* We don't care what the top level element name is */
    curNode = curNode->xmlChildrenNode;
    while (NULL != curNode) {

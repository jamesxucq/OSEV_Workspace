...
void event_dele(int epofd, struct event_data *evd) {
    struct epoll_event ev = {0, {0}};
    ev.data.ptr = evd;
...
#define EVENT_DELE(EPO_FD, EPO_LIST, EVD) { \
...
#define CHECK_TIMEOUT(EPO_FD, EPO_LIST, CHECK_TATOL, TIME_OUT, NOW) { \
    ...
    for (inde = 0, levd_list = EPO_LIST->chkpos; levd_list && inde<CHECK_TATOL; inde++, levd_list = (levd_head==next_elist?0L:next_elist)) { /* doesn't check listen fd */ \
        ...
        if ((NOW - levd_list->evd->last_active) >= TIME_OUT) { /* 60s timeout */ \
            ...
            EVENT_DELE(EPO_FD, EPO_LIST, levd_list->evd) \
...
/*_LOG_WARN("accept contro!");*/ \
    if(COLFD[0x00] == EVD->sockfd) \
        ACCEPT_CONNECT(EPO_FD, EPO_LIST, EVD, COLFD[0x00]) \
...
#define INITIAL_RECV_EVENT(EVD, EVDAT) \
    if(EVD->evstat) { \
        OSTP_EVDAT_BODY(EVDAT, EVD); \
        EVD->evstat = EVD->bev->handle[0x01](EVD->bev); \
    } else { \
/*_LOG_DEBUG("---- EVD->sockfd:%d EVD->evstat:%d", EVD->sockfd, EVD->evstat);*/ \
        OSTP_EVDAT_HDBD(EVDAT, EVD); \
        PROTO_METHOD(EVD->bev->method, EVDAT->type_ptr); \
/*_LOG_DEBUG("---- method:%d", EVD->bev->method);*/ \
        INITIAL_EVENT_HANDLE(EVD->bev->handle, EVD->bev->method) \
/*_LOG_DEBUG("---- EVD->bev->handle[0x00]:%X", EVD->bev->handle[0x00]);*/ \
        EVD->evstat = EVD->bev->handle[0x00](EVD->bev); \
...
/*_LOG_WARN("[fd:%d] recv over, modify to send.", EVD->sockfd);*/ \
            ...
            EVENT_DELE(EPO_FD, EPO_LIST, EVD) \
            break; \
        } else { \
            if ((EAGAIN != errno) && (EINTR != errno)) { \
...
int epoll_server(struct serv_config *config) {
    ...
    // create ctrl fd
    if (init_contro_event(colfd, epofd, &evds[max_events + 1], &evds[max_events + 2], time_out)) {
        dele_evd_list(elist);
        DELETE_EVENTS(events);
        final_event_data(evds, max_events);
        DELETE_EVDATS(evds);

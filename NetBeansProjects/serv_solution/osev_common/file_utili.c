...
int recu_mkdir(const char *dirpath, mode_t mode) {
    ...
    if (!mkdir(dirpath, mode)) return 0x00;
    if (ENOENT == errno) {
...
FILE *creat_bakup_file(FILE *local_stre, uint64 offset) {
    ...
    //
    do {
        rlen = fread(buffer, BUFF_SIZE, 0x01, local_stre);
        if (0 >= rlen) continue;
        //
        if (0 >= (wlen = fwrite(buffer, 0x01, rlen, bakup_stre))) continue;
...
int creat_action_bakup(char *location, char *file_name, time_t act_time) {
    ...
    if (!bakup) FAIL_EXIT(action, -1)
        //        
        do {
            rlen = fread(buffer, BUFF_SIZE, 0x01, action);
            if (0 >= rlen) continue;
            //
            if (0 >= (wlen = fwrite(buffer, 0x01, rlen, bakup))) continue;
...
char *load_data(char *data_cont, int cont_size, char *file_name) {
    struct stat64 statb;
    if (stat64(file_name, &statb) < 0) {
        _LOG_DEBUG("file_name:%s", file_name);
        _LOG_WARN("stat errno:%d", errno);
        return NULL;
    }
    if (cont_size < (statb.st_size + 0x01)) return NULL;
    //
    ...
    if (0 >= fread(data_cont, statb.st_size, 0x01, data_fp)) {
        fclose(data_fp);
        _LOG_ERROR("read file error! %s", file_name);
        return NULL;
    }
    data_cont[statb.st_size] = '\0';
    fclose(data_fp);
    //
    return data_cont;
}
...
int save_data(char *file_name, char *data_cont, int transa) {
    static char *modes[0x02] = {"wb+", "ab+"};
    int cont_size = strlen(data_cont);
    if (!cont_size) return 0x00;
    //
    FILE *data_fp = fopen64(file_name, modes[transa]);
    if (!data_fp) {

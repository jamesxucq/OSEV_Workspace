...
char *salt_auth_chks(char *chks_str, char *valid, char *salt) {
    char buffer[MID_TEXT_LEN];
    char chks[MD5_TEXT_LENGTH];
    //
    if (!valid || !chks_str) return NULL;
    sprintf(buffer, "%s:%s:%s", salt, valid, salt);
    printf("authen buffer:|%s|\n", buffer);
    get_md5sum_text(buffer, strlen(buffer), chks);
    //
    strcpy(chks_str, chks);
    return chks_str;
...
char *random_salt(char *salt_value) {
    ...
    last_seed = time(NULL);
    int tatol;
    for (tatol = 0; tatol < SALT_LENGTH; tatol++) {
        srand((unsigned int) last_seed);
        last_seed = rand();
        rand_str[tatol] = code_array[last_seed % 62];
    }
    // rand_str[tatol] = '\0';
    strncpy(salt_value, rand_str, SALT_LENGTH);
    return salt_value;
}
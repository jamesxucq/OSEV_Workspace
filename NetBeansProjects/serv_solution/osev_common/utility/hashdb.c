...
static int hashdb_swapout(HASHDB *hdb, uint32_t hash1, uint32_t hash2, HASH_ENTRY *he) {
    uint64_t root;
    uint32_t posi;
    int cmpv, lr = 0x00;
    HASH_ENTRY parent;
    ...
    if (0x00 == he->off) {
        ...
        while (root) {
            ...
            if (HASH_ENTRY_SZ != read(hdb->fd, &parent, HASH_ENTRY_SZ)) return -1;
            //
        ...
        if (parent.off) {
            ...
            if (HASH_ENTRY_SZ != write(hdb->fd, &parent, HASH_ENTRY_SZ)) return -1;
        }
    }
    /* flush cached hash entry to file */
    if (-1 == lseek(hdb->fd, he->off, SEEK_SET)) return -1;
    if (HASH_ENTRY_SZ != write(hdb->fd, he, HASH_ENTRY_SZ)) return -1;
    //
    he->off = 0x00;
    he->left = 0x00;
    he->right = 0x00;
...
static int hashdb_swapin(HASHDB *hdb, char *key, int ksiz, uint32_t hash1, uint32_t hash2, HASH_ENTRY *he) {
    uint32_t posi;
    uint64_t root;
    int cmpv;
    HASH_ENTRY entry;
    ...
    while (root) {
        if (-1 == lseek(hdb->fd, root, SEEK_SET)) return -1;
        if (HASH_ENTRY_SZ != read(hdb->fd, &entry, HASH_ENTRY_SZ)) return -1;
        //
        if (entry.hash > hash2) {
            root = entry.left;
            ...
            cmpv = memcmp(entry.key, key, ksiz);
            if (0x00 == cmpv) { /* find the entry */
                memcpy(he, &entry, HASH_ENTRY_SZ);
                he->cached = 0x01;
                return 0x00;
            } else if (cmpv > 0x00) {
                root = entry.left;
...
HASHDB *hashdb_new(uint64_t tnum, uint32_t bnum, uint32_t cnum, hashfunc_t hash_func1, hashfunc_t hash_func2) {
    HASHDB *hdb;
    if (!(hdb = malloc(sizeof (HASHDB)))) return NULL;
    //
...
static int hashdb_readahead(HASHDB *hdb) {
    ...
    for (inde = 0x00; inde < max_ent; inde++) {
        ...
        if (HASH_ENTRY_SZ != read(hdb->fd, &he, HASH_ENTRY_SZ)) return -1;
        //
        posi = hdb->hash_func1(he.key, he.ksiz) % hdb->header.cnum;
        memcpy(&hdb->cache[posi], &he, HASH_ENTRY_SZ);
...
int hashdb_open(HASHDB *hdb, const char *path) {
    ...
    //
    hdb->cache = (HASH_ENTRY *) malloc(hdb->header.cnum * HASH_ENTRY_SZ);
...
int hashdb_set(HASHDB *hdb, char *key, int ksiz, void *valu, int vsiz) {
    ...
    if (hdb->cache[posi].cached && ((hash2 != hdb->cache[posi].hash) || memcmp(key, hdb->cache[posi].key, ksiz))) {
        ...
        if (-1 == hashdb_swapout(hdb, he_hash1, he_hash2, &hdb->cache[posi])) return -1;
    }
    //
    if (!hdb->cache[posi].cached && bloom_check(hdb->bloom, 2, hash1, hash2)) {
        if (-1 == hashdb_swapin(hdb, key, ksiz, hash1, hash2, &hdb->cache[posi])) return -1;
    }
    //
    if ((HASHDB_KEY_MAX_SZ < ksiz) || (HASHDB_VALUE_MAX_SZ < vsiz)) return -1;
...
GET_TEST:
    ...
    for (inde = 0x00; inde <= max; inde++) {
        sprintf(key, "%d", inde);
        memset(valu, 0x00, HASHDB_VALUE_MAX_SZ);

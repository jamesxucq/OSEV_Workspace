...
#define OHMD_ENTRY_SZ sizeof(OHMD_ENTRY)
...
typedef struct {
    ...
    OHMD_ENTRY *cache; /* hash item cache */
    ohmdfunc_t ohmd_func1; /* hash function for hash bucket */
    ohmdfunc_t ohmd_func2; /* hash function for btree in the hash bucket */
} OIDHMD;

//
OIDHMD *oidhmd_new(uint64_t tnum, uint32_t bnum, uint32_t cnum, ohmdfunc_t ohmd_func1, ohmdfunc_t ohmd_func2); // ok
int oidhmd_open(OIDHMD *hmd, const char *path); // ok
int oidhmd_close(OIDHMD *hmd, int flash); // ok
int oidhmd_inse(OIDHMD *hmd, char *key);
int oidhmd_find(OIDHMD *hmd, char *key);
int oidhmd_unlink(OIDHMD *hmd); // ok

...
static int oidhmd_swapout(OIDHMD *hmd, uint32_t hash1, uint32_t hash2, OHMD_ENTRY *he) {
    uint64_t root;
    uint32_t posi;
    int cmpv, lr = 0x00;
    OHMD_ENTRY parent;
    ...
    if (0x00 == he->off) {
        ...
        while (root) {
            if (-1 == lseek(hmd->fd, root, SEEK_SET)) return -1;
            if (OHMD_ENTRY_SZ != read(hmd->fd, &parent, OHMD_ENTRY_SZ)) return -1;
            //
        ...
        /* append mode */
        if (-1 == (he->off = lseek(hmd->fd, 0x00, SEEK_END))) return -1;
        ...
        if (parent.off) {
            ...
            if (-1 == lseek(hmd->fd, parent.off, SEEK_SET)) return -1;
            if (OHMD_ENTRY_SZ != write(hmd->fd, &parent, OHMD_ENTRY_SZ)) return -1;
        }
    }
    /* flush cached hash entry to file */
    if (-1 == lseek(hmd->fd, he->off, SEEK_SET)) return -1;
    if (OHMD_ENTRY_SZ != write(hmd->fd, he, OHMD_ENTRY_SZ)) return -1;
    //
    he->off = 0x00;
    he->left = 0x00;
    he->right = 0x00;
...
static int oidhmd_swapin(OIDHMD *hmd, char *key, uint32_t hash1, uint32_t hash2, OHMD_ENTRY *he) {
    uint32_t posi;
    uint64_t root;
    int cmpv;
    OHMD_ENTRY entry;
    ...
    while (root) {
        if (-1 == lseek(hmd->fd, root, SEEK_SET)) return -1;
        if (OHMD_ENTRY_SZ != read(hmd->fd, &entry, OHMD_ENTRY_SZ)) return -1;
        //
        if (entry.hash > hash2) {
            root = entry.left;
            ...
            cmpv = memcmp(entry.key, key, OHMD_KEY_SZ);
            if (0x00 == cmpv) { /* find the entry */
                memcpy(he, &entry, OHMD_ENTRY_SZ);
                he->cached = 0x01;
                return 0x00;
            } else if (cmpv > 0x00) {
                root = entry.left;
...
OIDHMD *oidhmd_new(uint64_t tnum, uint32_t bnum, uint32_t cnum, ohmdfunc_t ohmd_func1, ohmdfunc_t ohmd_func2) {
    OIDHMD *hmd;
    if (!(hmd = malloc(sizeof (OIDHMD)))) return NULL;
    //
...
static int oidhmd_readahead(OIDHMD *hmd) {
    uint32_t inde, max_ent, posi;
    OHMD_ENTRY he;
    //
    if (!hmd || !hmd->fd || !hmd->bloom || !hmd->bucket || !hmd->cache) return -1;
    //
    max_ent = (hmd->header.cnum < hmd->header.bnum) ? hmd->header.cnum : hmd->header.bnum;
    for (inde = 0x00; inde < max_ent; inde++) {
        if (-1 == lseek(hmd->fd, hmd->bucket[inde].off, SEEK_SET)) return -1;
        if (OHMD_ENTRY_SZ != read(hmd->fd, &he, OHMD_ENTRY_SZ)) return -1;
        //
        posi = hmd->ohmd_func1(he.key, OHMD_KEY_SZ) % hmd->header.cnum;
        memcpy(&hmd->cache[posi], &he, OHMD_ENTRY_SZ);
...
int oidhmd_open(OIDHMD *hmd, const char *path) {
    int f_ok;
    uint64_t inde;
    ssize_t rwsiz;
    //
    ...
    for (inde = 0x00; inde < hmd->header.bnum; inde++) hmd->bucket[inde].off = 0x00;
    //
    hmd->cache = (OHMD_ENTRY *) malloc(hmd->header.cnum * OHMD_ENTRY_SZ);
    if (!hmd->cache) goto _ERRO_RET;
    for (inde = 0x00; inde < hmd->header.cnum; inde++) {
...
int oidhmd_close(OIDHMD *hmd, int flash) {
    uint64_t inde;
    uint32_t hash1, hash2;
    ssize_t wsiz;
    int retv = 0x00;
    //
    if (!hmd || !hmd->bloom || !hmd->bucket || !hmd->cache) return -1;
...
int oidhmd_inse(OIDHMD *hmd, char *key) {
    int posi;
    uint32_t hash1, hash2;
    ...
    if (hmd->cache[posi].cached && ((hash2 != hmd->cache[posi].hash) || memcmp(key, hmd->cache[posi].key, OHMD_KEY_SZ))) {
        ...
        if (-1 == oidhmd_swapout(hmd, he_hash1, he_hash2, &hmd->cache[posi])) return -1;
    }
    //
    if (!hmd->cache[posi].cached && bloom_check(hmd->bloom, 2, hash1, hash2)) {
        if (-1 == oidhmd_swapin(hmd, key, hash1, hash2, &hmd->cache[posi])) return -1;
    }
    // if ((OHMD_KEY_SZ < ksiz) || (OHMD_VALUE_SZ < vsiz)) return -1;
    /* fill up cache hash entry */
    memcpy(hmd->cache[posi].key, key, OHMD_KEY_SZ);
    hmd->cache[posi].hash = hash2;
    if (!hmd->cache[posi].cached) { /* it's a new entry */
...
int oidhmd_find(OIDHMD *hmd, char *key) {
    int posi, retv;
    uint32_t hash1, hash2;
    uint32_t he_hash1, he_hash2;
    //
...
GET_TEST:
    ...
    for (inde = 0x00; inde <= max; inde++) {
        sprintf(key, "%d", inde);
        memset(valu, 0x00, OHMD_VALUE_SZ);

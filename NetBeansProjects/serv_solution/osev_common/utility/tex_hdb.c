...
static int texhdb_swapout(TEXHDB *hdb, uint32_t hash1, uint32_t hash2, TEXH_ENTRY *he) {
    ...
    if (0x00 == he->off) {
        hebuf_sz = TEXH_ENTRY_SZ + TEXHDB_KEY_MAX_SZ + TEXHDB_VALUE_MAX_SZ;
        if (!(hebuf = (void *) malloc(hebuf_sz))) return -1;
        ...
        while (root) {
            ...
            // memset(hebuf, 0x00, hebuf_sz);
            if (hebuf_sz != read(hdb->fd, hebuf, hebuf_sz)) {
                free(hebuf);
                return -1;
            }
            //
            parent = (TEXH_ENTRY *) hebuf;
            hkey = (char *) (hebuf + TEXH_ENTRY_SZ);
            hvalu = (void *) (hebuf + TEXH_ENTRY_SZ + TEXHDB_KEY_MAX_SZ);
        ...
        if (parent->off) {
            ...
            if (TEXH_ENTRY_SZ != write(hdb->fd, parent, TEXH_ENTRY_SZ)) return -1;
        }
    }
    /* flush cached hash entry to file */
    if (-1 == lseek(hdb->fd, he->off, SEEK_SET)) return -1;
    if (TEXH_ENTRY_SZ != write(hdb->fd, he, TEXH_ENTRY_SZ)) return -1;
    memcpy(key, he->key, he->ksiz);
    if (TEXHDB_KEY_MAX_SZ != write(hdb->fd, key, TEXHDB_KEY_MAX_SZ)) return -1;
    memcpy(valu, he->valu, he->vsiz);
    if (TEXHDB_VALUE_MAX_SZ != write(hdb->fd, valu, TEXHDB_VALUE_MAX_SZ)) return -1;
    //
    if (he->key) {
        free(he->key);
        he->key = NULL;
...
static int texhdb_swapin(TEXHDB *hdb, char *key, int ksiz, uint32_t hash1, uint32_t hash2, TEXH_ENTRY *he) {
    uint32_t posi;
    uint64_t root;
    int cmpv, hebuf_sz;
    char *hkey;
    ...
    while (root) {
        ...
        if (entry->hash > hash2) {
            ...
            cmpv = memcmp(hkey, key, entry->ksiz > ksiz ? entry->ksiz : ksiz);
            if (0x00 == cmpv) { /* find the entry */
                memcpy(he, hebuf, TEXH_ENTRY_SZ);
                if (!(he->key = malloc(he->ksiz))) return -1;
                memcpy(he->key, hkey, he->ksiz);
                if (!(he->valu = malloc(he->vsiz))) return -1;
...
TEXHDB *texhdb_new(uint64_t tnum, uint32_t bnum, uint32_t cnum, texhfunc_t texh_func1, texhfunc_t texh_func2) {
    TEXHDB *hdb;
    if (!(hdb = malloc(sizeof (TEXHDB)))) return NULL;
    //
...
static int texhdb_readahead(TEXHDB *hdb) {
    ...
    for (inde = 0x00; inde < max_ent; inde++) {
        ...
        if (TEXH_ENTRY_SZ != read(hdb->fd, &he, TEXH_ENTRY_SZ)) return -1;
        if (TEXHDB_KEY_MAX_SZ != read(hdb->fd, key, TEXHDB_KEY_MAX_SZ)) return -1;
        if (TEXHDB_VALUE_MAX_SZ != read(hdb->fd, valu, TEXHDB_VALUE_MAX_SZ)) return -1;
        //
        posi = hdb->texh_func1(key, he.ksiz) % hdb->header.cnum;
        memcpy(&hdb->cache[posi], &he, TEXH_ENTRY_SZ);
        if (!(hdb->cache[posi].key = malloc(he.ksiz))) return -1;
        memcpy(hdb->cache[posi].key, key, he.ksiz);
        if (!(hdb->cache[posi].valu = malloc(he.vsiz))) return -1;
...
int texhdb_open(TEXHDB *hdb, const char *path) {
    ...
    //
    hdb->cache = (TEXH_ENTRY *) malloc(hdb->header.cnum * TEXH_ENTRY_SZ);
...
int texhdb_set(TEXHDB *hdb, char *key, int ksiz, void *valu, int vsiz) {
    ...
    if ((hdb->cache[posi].cached) && ((hash2 != hdb->cache[posi].hash) || (ksiz != hdb->cache[posi].ksiz) || memcmp(key, hdb->cache[posi].key, ksiz))) {
        ...
        if (-1 == texhdb_swapout(hdb, he_hash1, he_hash2, &hdb->cache[posi])) return -1;
    }
    //
    if (!hdb->cache[posi].cached && (bloom_check(hdb->bloom, 2, hash1, hash2))) {
        if (-1 == texhdb_swapin(hdb, key, ksiz, hash1, hash2, &hdb->cache[posi])) return -1;
    }
    //
    if ((TEXHDB_KEY_MAX_SZ < ksiz) || (TEXHDB_VALUE_MAX_SZ < vsiz)) return -1;
    /* fill up cache hash entry */
    if (hdb->cache[posi].key) free(hdb->cache[posi].key);
    if (hdb->cache[posi].valu) free(hdb->cache[posi].valu);
    //
    if (!(hdb->cache[posi].key = malloc(ksiz))) return -1;
    memcpy(hdb->cache[posi].key, key, ksiz);
    hdb->cache[posi].ksiz = ksiz;
    if (!(hdb->cache[posi].valu = malloc(vsiz))) return -1;
...
GET_TEST:
    ...
    for (inde = 0x00; inde <= max; inde++) {
        sprintf(key, "%d", inde);
        memset(valu, 0x00, TEXHDB_VALUE_MAX_SZ);

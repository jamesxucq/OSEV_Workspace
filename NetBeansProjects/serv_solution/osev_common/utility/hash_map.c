...
inline struct map_nod *node_dele(struct map_nod *nod, void (*freefunc)(void *)) {
...
int mlist_remove(struct map_nod **root, unsigned int key, void (*freefunc)(void *)) {
    ...
    for (posi = prev->next; NULL != posi; posi = posi->next) {
        if (posi->key == key) {
            prev->next = node_dele(posi, freefunc);
            return 0;
        }
        prev = posi;
    }
...
void hashm_dele(hashmap *hmap) {
    unsigned int inde;
    struct map_nod *posi;
    //
    if (NULL == hmap) return;
    for (inde = 0; inde < hmap->hashsize; inde++)
        for (posi = hmap->hmaplist[inde]; NULL != posi; posi = node_dele(posi, hmap->freefunc));
    //
    free(hmap->hmaplist);
    free(hmap);
}

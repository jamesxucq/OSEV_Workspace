...
inline struct acti_node *anod_dele(struct acti_node *nod, void (*freefunc)(void *)) {
...
void action_dele(actionmap *amap) {
    unsigned int inde;
    struct acti_node *posi;
    //
    if (NULL == amap) return;
    for (inde = 0; inde < amap->hashsize; inde++)
        for (posi = amap->hmaplist[inde]; NULL != posi; posi = anod_dele(posi, amap->freefunc));
    //
    free(amap->hmaplist);
    free(amap);
}

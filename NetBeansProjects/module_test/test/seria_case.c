...
void _bind_test_(riv_curso_t *curso, struct test *obje_data) {
    cursor->set_key(cursor, obje_data->obje_oid.stoid);
    cursor->set_value(cursor, obje_data->xx, obje_data->ax, obje_data->cx);
}

void _solve_test_(struct test *obje_data, riv_curso_t *curso) {
    cursor->get_key(cursor, &(obje_data->obje_oid.stoid));
    cursor->get_value(cursor, &obje_data->xx, &obje_data->ax, &obje_data->cx);
}
